...
- description：简短描述，使用小写英文或中文拼音，单词间用连字符 `-` 分隔
...
- 添加合适的 label（如 `类型: Feature`、`优先级: 高`）
...
5. **合并策略**：优先使用 squash merge 保持主分支历史清晰
...
mod tests {
    ...
    fn test_app_error_serde_roundtrip() {
        let error = AppError::NetworkError("连接超时".to_string());
...
**功能验证**
- [ ] 本地运行 `cargo check` 和 `cargo test` 通过
